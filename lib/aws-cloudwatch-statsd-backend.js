...
CloudwatchBackend.prototype.flush = function(timestamp, metrics) {
  ...
  for (var key in counters) {
      if (counters.hasOwnProperty(key)) {
        ...
        self.put_metric_data(key, counters[key], 'Count');
      }
  ...
  for (key in timers) {
    if (timers.hasOwnProperty(key) && timers[key].length > 0) {
      ...
      self.put_metric_data(key, null, 'Milliseconds', {
        Minimum: min,
        Maximum: max,
        Sum: sum,
        SampleCount: count
  ...
  for (key in gauges) {
    if (gauges.hasOwnProperty(key)) {
      self.put_metric_data(key, gauges[key]);
    }
  }

  for (key in sets) {
    if (sets.hasOwnProperty(key)) {
      self.put_metric_data(key, sets[key].values().length);
    }
  }
};
