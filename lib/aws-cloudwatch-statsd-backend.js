...
CloudwatchBackend.prototype.put_metric_data = function (timestamp, key, value, unit, stats) {
...
CloudwatchBackend.prototype.flush = function(timestamp, metrics) {
  ...
  console.log('Flushing metrics at ' + ts);
  ...
  for (var key in counters) {
      if (counters.hasOwnProperty(key)) {
        ...
        this.put_metric_data(timestamp, key, counters[key], 'Count');
      }
  ...
  for (key in timers) {
    if (timers.hasOwnProperty(key) && timers[key].length > 0) {
      ...
      this.put_metric_data(timestamp, key, null, 'Milliseconds', {
        Minimum: min,
        Maximum: max,
        Sum: sum,
        SampleCount: count
  ...
  for (key in gauges) {
    if (gauges.hasOwnProperty(key)) {
      this.put_metric_data(timestamp, key, gauges[key]);
    }
  }

  for (key in sets) {
    if (sets.hasOwnProperty(key)) {
      this.put_metric_data(timestamp, key, sets[key].values().length);
    }
  }
};
